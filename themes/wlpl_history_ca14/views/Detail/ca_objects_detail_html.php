...
/* WLPL custom code to pull our Library of Congress Subject Headings and custom keywords.
*/ 			# --- keyword terms
			$va_keywords = $t_object->get("ca_objects.subject", array('returnAsArray' => true, 'returnAllLocales' => false));

			if(sizeof ($va_keywords) > 0){
				print "<div class= 'unit'><h2>"._t("Keyword").((sizeof($va_keywords) > 1) ? "s" : "")." </h2>";
				foreach($va_keywords as $va_keyword) {
					print "<div>".caNavLink($this->request, $va_keyword['Subject'], '', '', 'Search', 'Index', array('search' => 'ca_objects.subject:"'.trim($va_keyword['Subject']).'"'))."</div>";
		}
			print "</div><!-- end unit -->";
			} 
